...
public class LogFindServletImpl implements LogFindServlet {
    ...
    public PageBean<ManagerUser_log> FindlogByPage(String _currentPage, String _rows, Map<String, String[]> parameterMap) {
        ...
        //查询总记录数
        int totalcount = logListServlet.findManagerTotalCount(parameterMap);
        ...
        List<ManagerUser_log> byPage = logListServlet.findManagerByPage(start, rows, parameterMap);
        pb.setList(byPage);
        return pb;
    }

    @Override
    public PageBean<SurperUser_log> FindSlogByPage(String _currentPage, String _rows, Map<String, String[]> parameterMap) {
        //防止空指针异常
        if (_currentPage == null) {
            _currentPage = "1";
        }
        ...
        int currentPage = Integer.parseInt(_currentPage);
        int rows = Integer.parseInt(_rows);
        PageBean<SurperUser_log> pb = new PageBean<SurperUser_log>();
        //查询总记录数
        int totalcount = logListServlet.findSurperTotalCount(parameterMap);
        pb.setTotalcont(totalcount);
        //计算总页码
        int totalPage = totalcount % rows == 0 ? totalcount / rows : totalcount / rows + 1;
        pb.setTotal(totalPage);
        ...
        //需要查询开始页面的索引
        int start = (currentPage - 1) * rows;
        List<SurperUser_log> byPage = logListServlet.findSurperByPage(start, rows, parameterMap);
        pb.setList(byPage);
        return pb;
    }
}
